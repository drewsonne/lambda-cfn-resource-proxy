...
install: build
	...
	$(ENVS) aws cloudformation create-stack \
		--template-body file://$(CURDIR)/custom_$(PRODUCT).json \
		--stack-name CFNRscProxy \
	...
	$(ENVS) aws cloudformation wait stack-create-complete --stack-name CFNRscProxy

demo:
	$(ENVS) aws cloudformation create-stack \
		--template-body file://$(CURDIR)/demo_$(PRODUCT).json \
		--stack-name CFNDemo \
		--parameters \
